...
app.post("/saarofoods/download-pdf", (req, res) => {
  ...
  doc.fillColor("#004d40")
     .fontSize(10) // GST-IN on the left
  ...
  customer.Products.forEach((product, index) => {
    ...
    if (position + 25 > doc.page.height - 100) {
  ...
  // Ensure enough space for total calculation
  ...
  const finalAmount = totalAmount - discount;  // No delivery charge, so just subtract discount
